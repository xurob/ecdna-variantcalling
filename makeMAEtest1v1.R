...
importMito.explicit <- function(Afile, Cfile, Gfile, Tfile,
  ...
  importSMs <- function(file){
    ...
    counts2 <- Matrix::sparseMatrix(
      i = c(dt[[1]],maxpos),
      j = c(as.numeric(dt[[2]]), maxsamples),
      x = c(dt[[3]],0)
    )
    counts <- counts2[minpos:maxpos,, drop = FALSE]
    
    BAQ2 <- Matrix::sparseMatrix(
      i = c(dt[[1]],maxpos),
      j = c(as.numeric(dt[[2]]), maxsamples),
      x = c(dt[[4]],0)
    )
    BAQ <- BAQ2[minpos:maxpos,, drop = FALSE]
  ...
  ref <- data.frame(
    V1 = minpos:maxpos,
    V2 = toupper(unname(reffasta)[[1]])[minpos:maxpos]
  )
  ref
  size <- maxpos - minpos +1
  # Make a long matrix of BAQ and Counts for non-reference alleles
  whichA <- which(ref[["V2"]][1:size] != "A")
  whichC <- which(ref[["V2"]][1:size] != "C")
  whichG <- which(ref[["V2"]][1:size] != "G")
  whichT <- which(ref[["V2"]][1:size] != "T")
  whichT
  ...
  row_g_cov <- GenomicRanges::GRanges(seqnames = mitoChr,
                                      IRanges::IRanges(1:size, width = 1))
  GenomicRanges::mcols(row_g_cov) <- data.frame(refAllele = ref[[2]][1:size])
  
  row_g_allele <- GenomicRanges::GRanges(seqnames = mitoChr,
                                         IRanges::IRanges(1:size, width = 1))[c(whichA, whichC, whichG, whichT)]
  
  GenomicRanges::mcols(row_g_allele) <- data.frame(refAllele = (ref[[2]][1:size])[c(whichA, whichC, whichG, whichT)],
                                                   altAllele = letterz)
